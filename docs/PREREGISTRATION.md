...
- Runs launched before the v1 freeze (stack checks, calibrations, speed checks, gate 1) are exploratory and are labelled as such in the paper. One disclosed exception: `gate1-c2-paired-s0` has exactly the spec of C2 paired seed 0 (same environment, data, schedules, trainer settings and evaluation protocol) and is included as that run in tier 1. Its validation curve and final sets were known when T and the funded list were set; the decision rules of section 3 were fixed in v0.2 before it finished and are applied to all three seeds without change. The gradient probe on its checkpoints runs after the freeze.
